...
async def mergeSub(c: Client, cb: CallbackQuery, new_file_name: str):
    ...
    vid_list = list()
    await cb.message.edit("⚡ ᴘʀᴏᴄᴇssɪɴɢ...")
    duration = 0
    ...
    for i in msgs:
        ...
        await cb.message.edit(f"⚡ sᴛᴀʀᴛɪɴɢ ᴅᴏᴡᴏᴀᴅ ᴏғ ... `{media.file_name}`")
        ...
        try:
            ...
            file_dl_path = await c.download_media(
                ...
                progress_args=(f"⚡ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ..: `{media.file_name}`", c_time,f"\n**Downloading: {n}/{all}**"),
            )
            n+=1
            if gDict[cb.message.chat.id] and cb.message.id in gDict[cb.message.chat.id]:
                return
            await cb.message.edit(f"ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ sᴜᴄᴄᴇsғᴜʟʟʏ ... `{media.file_name}`")
            LOGGER.info(f"ᴅᴏᴡɴʟᴏᴀᴅ sᴜᴄᴄᴇsғᴜʟʟʏ ... {media.file_name}")
            await asyncio.sleep(5)
        except Exception as downloadErr:
            LOGGER.warning(f"Failed to download Error: {downloadErr}")
            queueDB.get(cb.from_user.id)["subtitles"].remove(i.id)
            await cb.message.edit("❗ғɪʟᴇ sᴋɪᴘᴘᴇᴅ!")
    ...
    if subbed_video is None:
        await cb.message.edit("❌ ғᴀɪʟᴇᴅ ᴛᴏ ᴀᴅᴅ sᴜʙs ɪɴ ᴠɪᴅᴇᴏ !")
        await delete_all(root=f"downloads/{str(cb.from_user.id)}")
        queueDB.update({cb.from_user.id: {"videos": [], "subtitles": [], "audios": []}})
        formatDB.update({cb.from_user.id: None})
        return
    try:
        await cb.message.edit("✅ sᴜᴄᴄᴇsғᴜʟʟʏ ᴍᴜxᴇᴅ ᴠɪᴅᴇᴏ !")
    except MessageNotModified:
        await cb.message.edit("sᴜᴄᴄᴇssғᴜʟʟʏ ᴍᴜxᴇᴅ ᴠɪᴅᴇᴏ ! ✅")
    ...
    if Config.IS_PREMIUM and file_size > 4241280205:
        ...
        formatDB.update({cb.from_user.id: None})
        return
    await cb.message.edit("🎥 ᴇxᴛʀᴀᴄᴛɪɴɢ ᴠɪᴅᴇᴏ ᴅᴀᴛᴀ ...")
    ...
    try:
        ...
        queueDB.update({cb.from_user.id: {"videos": [], "subtitles": [], "audios": []}})
        formatDB.update({cb.from_user.id: None})
        await cb.message.edit("⭕ ᴍᴇʀɢᴇᴅ ᴠɪᴅᴇᴏ ɪs ᴄᴏʀʀᴜᴘᴛᴇᴅ")
        return
    try:
    ...
    try:
        ...
        formatDB.update({cb.from_user.id: None})
        await cb.message.edit(
            "⭕ ᴍᴇʀɢᴇᴅ ᴠɪᴅᴇᴏ ɪs ᴄᴏʀʀᴜᴘᴛᴇᴅ \n\n<i>ᴛʀʏ sᴇᴛᴛɪɴɢ ᴄᴜsᴛᴏᴍ ᴛʜᴜᴍʙɴᴀɪʟ</i>",
        )
        return
    await uploadVideo(
        c=c,
